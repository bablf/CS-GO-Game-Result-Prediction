...
def cleanCSV(csv):
    """
    This function deletes outdated rows from the CSV and returns the latest row
    """
    pass
...
def parsePage(url):
    ...
    """if PARSE_TIMEOUT == 0 and PROXIES is None: # scrape proxies if not yet done
        soup = parsePage("https://www.sslproxies.org/")
        proxies_table = soup.find(id='proxylisttable')

        for row in proxies_table.tbody.find_all('tr'):
            PROXIES.append(row.find_all('td')[0].string + ":" + row.find_all('td')[1].string) # ip:port
    ...
    #proxy_index = random.randint(0, len(PROXIES) - 1) # setup proxy usage
...
def parsePastMatches(startDate, endDate, current_offset = -1):
    ...
    if (current_offset * 50) >= final_offset:
        return True
    ...
    try:
        ...
        last_match_urls = df.iloc[-100:, 2].values
    ...
    for match in match_urls:
        if "https://www.hltv.org/" + match in last_match_urls: # skip if already scraped
            if DEBUG >= 0:
                print("\n[" + datetime.now().strftime("%Y-%m-%d %H:%M:%S") + "] " + "Skipping match " + match + " because it is already saved.")
        ...
        if DEBUG >= 0:
            print("\n[" + datetime.now().strftime("%Y-%m-%d %H:%M:%S") + "] " + "[" + str(match_urls.index(match) + 1) + "/50 on page " + str(current_offset) + "] Parsing match " + team1 + " vs " + team2 + " on " + date + " at " + event + ".")
    ...
    return parsePastMatches("2017-01-12", "2020-03-14", current_offset)
