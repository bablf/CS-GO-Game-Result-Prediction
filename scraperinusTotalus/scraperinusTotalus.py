...
def parsePastMatches(startDate, endDate, current_offset=0):
    ...
    if (current_offset * 50) < final_offset:
        ...
        for match in match_urls:
            ...
            if DEBUG >= 0:
                print(DEBUG_LOG + "[" + str(match_urls.index(match) + 1) + "/50 on page " + str(current_offset) + "] Parsing match " + team1 + " vs " + team2 + " on " + date + " at " + event + ".")
            ...
            for player in players:
                ...
                if player_stats is None or player_stats_3m is None:
                    if DEBUG >= 0:
                        print(DEBUG_LOG + "[-] Not enough player data for match " + team1 + " vs " + team2 + " on " + date + " at " + event + ". Total matches parsed: " + str(match_count))
                    break
            ...
            if len(csv_row) == len(csv_headers): # add match row only if we have all data
                ...
                with open(sys.path[0] + '/past_matches.csv', 'a', newline='') as f:
                    ...
                    if DEBUG >= 0:
                        print(DEBUG_LOG + "[" + str(match_urls.index(match) + 1) + "/50] on page " + str(current_offset) + "] [+] Saved match " + team1 + " vs " + team2 + " on " + date + " at " + event + ". Total matches parsed: " + str(match_count))
        ...
        parsePastMatches("2017-01-12", "2020-03-14", current_offset)
